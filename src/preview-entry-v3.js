...
async function validateMiroRequest(request, env, ctx) {
  ...
  return await previewWorker.fetch(probeRequest, env, ctx);
}
...
async function searchJiraUsers(env, query) {
  const jiraBase =
  ...
  let response = await fetch(
    `${jiraBase}/user/search?query=${encodeURIComponent(query)}&maxResults=50`,
    { headers },
  );

  if (!response.ok) {
    response = await fetch(
      `${jiraBase}/user/assignable/search?project=SN&query=${encodeURIComponent(query)}&maxResults=50`,
      { headers },
    );
  }
...
async function findJiraReporter(env, creator) {
  ...
  const search = await searchJiraUsers(env, creator.name);
  if (!search.ok) {
    return {
      ok: false,
      stage: "reporter-search-jira-user",
      jiraStatus: search.jiraStatus,
      error: search.error,
...
async function createJiraIssueWithReporter(env, requestBody, reporter) {
  const summary = String(requestBody?.summary ?? "")
    .replace(/\s+/g, " ")
    .trim();
  ...
  if (summary.length > 255) {
    return {
      ...
      body: {
        ok: false,
        reason: "Sticky text is too long for Jira summary",
        maxLength: 255,
      },
    };
  }
  ...
  if (!allowedWorkTypes.has(workType)) {
    return {
      status: 400,
      body: { ok: false, reason: "Unapproved work type", workType },
    };
  }
  ...
  const issueTypes = Array.isArray(issueTypesData?.issueTypes)
    ? issueTypesData.issueTypes
    : [];
  const matchingIssueType = issueTypes.find(
    item =>
      String(item?.name ?? "").trim().toLowerCase() === workType.toLowerCase(),
  );
  ...
  if (workType === "Bug") {
    ...
    const customerOption = customerField
      ? findDropdownOption(customerField, defaultText)
      : null;
  ...
  if (!createResponse.ok) {
    ...
    return {
      status:
        createResponse.status >= 400 && createResponse.status < 500
...
export default {
  async fetch(request, env, ctx) {
    ...
    const creation = await createJiraIssueWithReporter(
      env,
      requestBody,
      reporterResult,
    );
    ...
    console.log(
      "MIRO STICKY CREATOR -> JIRA REPORTER AT CREATE:",
      {
