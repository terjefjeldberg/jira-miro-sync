export function renderMiroPanelV3() {
...
<body>
  ...
  <p>Select one or more sticky notes. Each note is replaced by a Jira card in exactly the same position. Notes inside a workflow column get that Jira status; notes outside the workflow board keep Jira's default status.</p>
  <div class="card">
    ...
    <details>
      ...
      <div class="admin-tools">
        ...
        <div id="creatorDiagnostic" class="diagnostic"><label for="creatorId">Miro creator ID</label><input id="creatorId" readonly /></div>
        <button id="frameIdButton" type="button">Show selected frame ID</button>
        <div id="frameDiagnostic" class="diagnostic"><label for="frameId">Miro frame ID</label><input id="frameId" readonly /></div>
      </div>
    </details>
  </div>
  <script src="/miro-panel-v3-client.js?v=20260831-1"></script>
</body>
</html>`, {
    headers: {
...
export function renderMiroPanelV3Client() {
  ...
  async function createJiraIssue(sticky, summary, workType) {
    ...
    return String(result.issueKey);
  }

  async function createDirectCard(issueKey, pos) {
    const response = await backendPost("/conversion-direct-card", { issueKey: issueKey, x: pos.x, y: pos.y });
    let result = null;
    try { result = await response.json(); } catch (error) {}
    if (!response.ok || !result || !result.ok || !result.itemId) {
      const reason = result && (result.reason || result.error) || "Could not create Miro card at the note position.";
      throw new Error(typeof reason === "string" ? reason : JSON.stringify(reason));
    }
  ...
  async function convertSticky(sticky) {
    ...
    if (!issueKey) {
      issueKey = await createJiraIssue(sticky, summary, workType);
    ...
    const itemId = await createDirectCard(issueKey, originalPosition);
    await applyConversionStatus(issueKey, desiredStatus);

    await sticky.setMetadata(CONVERSION_METADATA_KEY, {
      issueKey: issueKey,
      stage: "card-created",
      customCardFrameId: itemId,
      workType: workType,
      stickyColor: stickyColor,
      desiredStatus: desiredStatus || null
    });
    ...
    return { issueKey: issueKey, desiredStatus: desiredStatus };
  }
  ...
  convertButton.addEventListener("click", async function () {
    ...
    try {
      ...
      if (!failures.length) {
        setStatus(successes.length === 1 ? successes[0].issueKey + " converted successfully." : successes.length + " notes converted successfully.", "success");
